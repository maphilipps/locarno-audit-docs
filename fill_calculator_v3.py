...
def main():
    ...
    # Remove charts to avoid openpyxl serialization issues
    print(f"\n🗑️  Removing charts (workaround for openpyxl compatibility)...")
    for sheet_name in wb.sheetnames:
        sheet = wb[sheet_name]
        sheet._charts = []

    print(f"💾 Saving as .xlsx (without macros and charts): {output_path.name}")
    wb.save(str(output_path))
    print(f"✅ Successfully saved to: {output_path}")
    print(f"\n⚠️  NOTE: Charts were removed due to Excel compatibility issues")
    print(f"   Open the original template to view charts with the new data")
    return 0
